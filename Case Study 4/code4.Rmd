...
PredictLogGain <- function(density)
  predict(least.squares, data.frame(density=density))  # Return log(gain)

PredictDensity <- function(gain)
  (log(gain) - intercept) / slope  # Return density
...
Pi <- function(density) {
  ...
  c(center - width, center + width)  # Return prediction interval
}
...
Ci <- function(density) {
  ...
  c(center - width, center + width)  # Return confidence interval
}
...
plot(df.log, main=title, xlab=x.axis, ylab=y.log.axis)
abline(least.squares, col='red')
curve(f, n=2, col='blue', add=TRUE)
legend('topright', legend=c('Least Squares', 'Upper PI'), col=c('red', 'blue'), lty=1)
```
