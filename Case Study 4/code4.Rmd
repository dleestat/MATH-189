...
plot(raw$density, raw$gain, main='Density vs. Gain', xlab=density, ylab=gain, col=color)
plot(raw$gain, raw$density, main='Gain vs. Density', xlab=gain, ylab=density, col=color)

density <- expression('log(Density) (g/cm'^3*')')

plot(log(raw$gain), raw$density, main='Gain vs. Density', xlab=gain, ylab=density, col=color)
...
library(data.table)
raw_cleaned<- raw[,.(mean(raw$gain)),by= .(raw$density)]
correlation <-cor(raw$density,raw$gain)
#least squares line
fit<-lm(raw$gain~raw$density)
summary(fit)
