...
Ci <- function(density) {
  ...
  list(center - width, center + width)  # Return confidence interval
}

Pi <- function(density) {
  center <- PredictLogGain(density)
  width <- t * s.pooled * sqrt(1 + 1/m + (density-mean.density)^2 / summation)
  list(center - width, center + width)  # Return prediction interval
}
...
confidence.intervals <- data.frame(density=df.log.avg$density, lower=Ci(df.log.avg$density)[[1]], upper=Ci(df.log.avg$density)[[2]])
...
prediction.intervals <- data.frame(density=df.log.avg$density, lower=Pi(df.log.avg$density)[[1]], upper=Pi(df.log.avg$density)[[2]])
...
legend('topright', legend=c('Least Squares', 'Confidence Interval Bands', 'Prediction Interval Bands'), col=c('red', ci.col, pi.col), lty=1)
```
