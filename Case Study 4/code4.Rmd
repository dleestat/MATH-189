...
s2.pooled = mean(s2$variance)

Pi <- function(density) {
  center <- PredictLogGain(density)
  width <- qt(.975, df=m-2) * s2.pooled * sqrt(1 + 1/m + (density-mean.density)^2 / summation)
  c(center - width, center + width)
}

Ci <- function(density) {
  center <- PredictLogGain(density)
  width <- qt(.975, df=m-2) * s2.pooled * sqrt(1/m + (density-mean.density)^2 / summation)
  c(center - width, center + width)
}
```
