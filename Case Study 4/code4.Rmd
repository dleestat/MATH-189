...
df <- df[order(df$density),]  # Least to greatest density
...
LogPredict <- function(density) {
  predict(least.squares, data.frame(density=density))  # Returns log(gain)
}

Invert <- function(gain) {
  (log(gain) - intercept) / slope  # Returns density
}
