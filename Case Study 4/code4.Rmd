...
Pi <- function(density) {
  center <- PredictLogGain(density)
  width <- t * s.pooled * sqrt(1 + 1/m + (density-mean.density)^2 / summation)
  list(center - width, center + width)  # Return lower end of prediction interval
}

CiLower <- function(density) {
  center <- PredictLogGain(density)
  width <- t * s.pooled * sqrt(1/m + (density-mean.density)^2 / summation)
  center - width  # Return lower end of confidence interval
}

CiUpper <- function(density) {
  center <- PredictLogGain(density)
  width <- t * s.pooled * sqrt(1/m + (density-mean.density)^2 / summation)
  center + width  # Return lower end of confidence interval
}

PiLower <- Vectorize(PiLower)
plot(df.log, main=title, xlab=x.axis, ylab=y.log.axis)
abline(least.squares, col='red')
evaluations <- 100000
# curve(PiLower, n=evaluations, col='blue', add=TRUE)
prediction.intervals <- data.frame(density=df.log.avg$density, lower=)
lines(x=df.log.avg$density, y=prediction.intervals[[1]])
curve(PiUpper, n=evaluations, col='blue', add=TRUE)
curve(CiLower, n=evaluations, col='green', add=TRUE)
curve(CiUpper, n=evaluations, col='green', add=TRUE)
legend('topright', legend=c('Least Squares', 'PI', 'CI'), col=c('red', 'blue', 'green'), lty=1)
```
