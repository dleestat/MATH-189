...
## Setup
```{r}
raw <- read.table('gauge-1wb1wa6-2gpel41.txt', header=TRUE)
```
...
library(data.table)
data_cleaned<-aggregate(gain ~density, raw, mean)
# Plot raw data
density <- expression('Density (g/cm'^3*')')
gain <- 'Gain'
color <- 'red'

plot(data_cleaned$density, data_cleaned$gain, main='Density vs. Gain', xlab=density, ylab=gain, col=color)
plot(data_cleaned$gain, data_cleaned$density, main='Gain vs. Density', xlab=gain, ylab=density, col=color)

density <- expression('log(Density) (g/cm'^3*')')

plot(log(data_cleaned$gain), data_cleaned$density, main='Gain vs. Density', xlab=gain, ylab=density, col=color)
...
table.1 <- table.1[which(table.1$Ts>-200 & table.1$Hour>21),]
table.1 <- table.1[,c('DOY','Ts')]

# least squares line
fit<-lm(table.1$Ts~table.1$DOY)
summary(fit)

# Plot the data
plot(table.1, xlab="DOY", ylab="Ts", main="DOY vs Ts")
abline(coefficients((fit)),col="red")

#standardized residual
fit.std<- rstandard(fit)
yfit <- fit$fitted.values
resid<-table.1$Ts-yfit
plot(table.1$DOY,resid)
#best fit line on a residual plot
abline(0,0,col="red")

#use qqplot to find the normality of residual
qqnorm(fit.std,ylab="Standardized Residuals",xlab="Theoretical")
qqline(fit.std)
