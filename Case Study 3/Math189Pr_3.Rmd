...
>>>>>>> e347a4865a763b363b2b3512094a5426a7b97e69
...
## Chi-square Goodness of Fit Test
Case 1: Group locations into 100 sub-intervals
```{r}
k <- 100
locations.theo <- n/k
tab <- table(cut(pop, breaks = seq(0, N, length.out = k+1), include.lowest = TRUE))
...
distance <- abs(pop[-1]-pop[-length(pop)])
```
...
sort(distance, decreasing = FALSE)
```

```{r}
<<<<<<< HEAD
# Construct palindrome count number
palindrome.count <- c("1-88.4", "88.4-176","176-263","263-351","351-438","438-525","525-613", "613-700", "700-788", "788-875", "875-963", "963-1050", "1050-1140", "1140-1220")

# Construct observed number of intervals
intervals.observed <- c(51,28,27,26,13,15,15,10,7,9,7,9,11,13,9,6,11,7,5,5,6,6,9) 


# Construct expected number of intervals
expected <- c()
lambda <- sum(distance)/(n-1)
for (i in c(0:17)){
  expect <- k* exp(-lambda)
