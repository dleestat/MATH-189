...
The purple histogram represents the overlapped area.
```{r}
pop.sorted = sort(pop, decreasing = FALSE)
distance.pair <- abs(pop.sorted[-1]-pop.sorted[-length(pop)])
distance.pair
```
...
```{r}
k <- 61
spacings.obs <- distance.pair
sort(distance.pair, decreasing = FALSE)
```
