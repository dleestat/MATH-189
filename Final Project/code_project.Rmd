...
```{r}
df1 <- merge(projects.categorized, projects[, c(1,2,12,14)], by='Project ID')
df2 <- merge(df1, donations[, c(1,3,5)], by='Project ID')[, -1]
df3 <- merge(df2, donors[, c(1,4)], by='Donor ID')[, -1]
df <- merge(df3, schools[, c(1,4)], by='School ID')[, -1]
df <- df[`Project Grade Level Category` != 'unknown']
head(df, 100)
```
...
```{r}
literacy <- df
# literacy$`Project Subject Category Tree` <- literacy$`Project Subject Category Tree` == 'Literacy & Language'
literacy$`Donor Is Teacher` <- literacy$`Donor Is Teacher` == 'Yes'
literacy <- as.data.frame(literacy)

# category <- as.integer(literacy$`Project Subject Category Tree`)
# grade <- as.factor(literacy$`Project Grade Level Category`)
# project.cost <- as.numeric(literacy$`Project Cost`)
# donation.amount <- as.numeric(literacy$`Donation Amount`)
# donor.teacher <- as.numeric(literacy$`Donor Is Teacher`)
# free.lunch <- as.numeric(literacy$`School Percentage Free Lunch`)

# category~grade+project.cost+donation.amount+donor.teacher+free.lunch+project.cost
colnames(literacy) <- c('category', 'grade', 'cost', 'amt', 'teacher', 'lunch')
literacy <- na.omit(literacy)
category.tree <- randomForest(as.factor(category)~lunch, data=literacy)
unique(literacy$lunch)
unique(literacy$`Project Subject Category Tree`)
plot(category.tree)
text(category.tree)
```
...
```{r}
population <- data.table::fread("io/nst-est2017-01.csv")
