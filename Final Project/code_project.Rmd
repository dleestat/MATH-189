...
<<<<<<< HEAD:Final Project/code_final_project.Rmd
1. Regression modeling
2. Properties of Least Squares/Fitting "a line"
3. Multiple observation
4. Chi-Square Goodness of Fit Test
5. Hypothesis Testing: Wilcoxon Signed Rank Test
=======
***
>>>>>>> e62c8445efd683fc6601c4ac4ebde0e9c37e2678:Final Project/code_project.Rmd
...
<<<<<<< HEAD:Final Project/code_final_project.Rmd
1. The main file: df_merged.csv
2. Original Dataset: Donations.csv, Donors.csv, Teachers.csv, Resources.csv, Projects.csv, Schools.csv
=======
...
<<<<<<< HEAD:Final Project/code_final_project.Rmd
## Import Dataset and merge Dataset
=======
...
for (i in df_merged$Project.Subject.Category.Tree){
    a <- strsplit(i, ", ", fixed = FALSE, perl = FALSE, useBytes = FALSE)[[1]]
...
# Attempt to add feature columns to the merged dataset
for (i in unique_category){
  df_merged[,i] <- 0
  for (j in c(1:nrow(df_merged))){
    a <- grep(i, df_merged[j,'Project.Subject.Category.Tree'])
    if (length(a) != 0){
      df_merged[,i] <- 1
    }
  }
...
```{r}
teacher <- df_merged[which(`Donor.Is.Teacher` == 'Yes'),]
nonteacher <- df_merged[which(`Donor.Is.Teacher`=='No'),]
...
```{r}
sample <- merged[sample(nrow(merged), 10000), ]
...
rf.fit <- randomForest(donations ~ ., data = merged[train, ], importance = TRUE, ntree = 1000, nodesize = 10)
varImpPlot(rf.fit)
...
```{r}
# the color parameter needs to be changed
