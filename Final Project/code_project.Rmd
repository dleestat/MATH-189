...
```{r}
df1 <- merge(projects.categorized, projects[, c(1,2,3,12,13,14)], by='Project ID')
df2 <- merge(df1, donations[, c(1,3,5)], by='Project ID')
df3 <- merge(df2, donors[, c(1,4)], by='Donor ID')
unique(df3$`Project Resource Category`)
unique(projects$`Project Subject Category Tree`)
