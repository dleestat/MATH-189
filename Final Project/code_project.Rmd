...
```
```{r}
# Ensure there is only one subject category per row of Projects.csv for further analysis
# Take a random sample of Projects.csv with only Project ID and Project Subject Category Tree columns
# projects.categorized is a data.table with Project ID and a row for every unique subject category for that Project ID
set.seed(0)
n <- 50000
...
for (row in 1:nrow(projects.categorized)) {
  categories <- unlist(strsplit(projects.categorized[row]$`Project Subject Category Tree`, ', '))
  if (length(categories) > 1) {
    duplicate <- projects.categorized[row]
    projects.categorized[row]$`Project Subject Category Tree` <- categories[1]
...
```
```{r}
# 9 unique project subject categories
projects.categorized[, .N, by=`Project Subject Category Tree`][order(N, decreasing=TRUE)]
```
...
texts[`Project Subject Category Tree`=='Warmth']
```

```{r}
df <- merge(projects.categorized, projects[, -c(4,5,6,7,10,11,15,16,17,18)], by='Project ID')
df
